  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    try {
      ...
      if (data) {
        ...
        let currentAccountId = data.account_id ?? null;
        let currentAccountRole = accountRole;

        if (!currentAccountId || !currentAccountRole) {
          // Attempt automatic self-bootstrap via server admin client
          try {
            const bootRes = await fetch("/api/account/bootstrap", { method: "POST" });
            if (bootRes.ok) {
            ...
            // Ignore failure, fall through to status detail
          }
        }
        ...
        if (!currentAccountId || !currentAccountRole) {
          ...
          setStatusDetail(
            `profile ${data.id} has no ${!currentAccountId ? "account_id" : "account_role"}`,
          );
        }
      } else {
        // Table exists, but no row for this user. Try automatic bootstrap once.
        try {
          const bootRes = await fetch("/api/account/bootstrap", { method: "POST" });
          if (bootRes.ok) {
            const retryRes = await supabase
              ...
              .maybeSingle();
            if (retryRes.data) {
              const accountRole = isAccountRole(retryRes.data.account_role)
                ? retryRes.data.account_role
                : null;
              setProfile({
                id: retryRes.data.id,
                full_name: retryRes.data.full_name,
                email: retryRes.data.email,
                avatar_url: retryRes.data.avatar_url,
                role: retryRes.data.role,
                beta_features: retryRes.data.beta_features ?? [],
                account_id: retryRes.data.account_id ?? null,
                account_role: accountRole,
              });
              return;
            }
